...
class AppointmentBookingService:
    ...
    async def check_double_match(self, page: Page, reservation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if both_match:
            ...
            message = f"❌ Kapasite ({found_capacity}<{required_capacity}) ve tarih ({found_date}) uyumsuz"
    ...
    async def fill_applicant_form(self, page: Page, person: Dict[str, Any], index: int = 0) -> None:
        ...
        gender_selector = VFS_SELECTORS['gender_female'] if person['gender'].lower() == 'female' else VFS_SELECTORS['gender_male']
        ...
        # Passport number
        await self.human_type(page, VFS_SELECTORS['passport_number'], person['passport_number'].upper())
        
        # Passport expiry
        await self.human_type(page, VFS_SELECTORS['passport_expiry'], person['passport_expiry_date'])
