...
class AppointmentBookingService:
    ...
    async def check_double_match(self, page: Page, reservation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if both_match:
            message = (
                f"✅ Uygun randevu bulundu! {found_capacity} kişilik, {found_date}"
            )
    ...
    async def fill_applicant_form(self, page: Page, person: Dict[str, Any], index: int = 0) -> None:
        ...
        """
        logger.info(
            f"Filling form for person {index + 1}: "
    ...
    async def select_appointment_slot(self, page: Page, reservation: Dict[str, Any]) -> bool:
        ...
        # Note: preferred_dates from reservation could be used for validation
        # but current implementation selects first available date
