...
class SMSPayloadParser:
    ...
    def parse(cls, payload: Dict[str, Any]) -> SMSPayload:
        ...
        # Extract message
        message = next(
            (payload.get(field) for field in cls.MESSAGE_FIELDS if payload.get(field)), None
        )
        ...
        phone_number = next(
            (payload.get(field) for field in cls.PHONE_FIELDS if payload.get(field)), None
        )

        # Extract timestamp
        timestamp = next(
            (payload.get(field) for field in cls.TIMESTAMP_FIELDS if payload.get(field)), None
        )

        # Extract SIM slot
        sim_slot_raw = next(
            (payload.get(field) for field in cls.SIM_SLOT_FIELDS if payload.get(field)), None
        )
