...
class BookingOrchestrator:
    ...
    async def parse_aria_label_to_date(self, aria_label: str):
        """Delegate to slot_selector."""
        return self.slot_selector.parse_aria_label_to_date(aria_label)

    async def human_type(self, page: Page, selector_key: str, text: str):
        """Delegate to form_filler."""
        return await self.form_filler.human_type(page, selector_key, text)

    def normalize_date(self, date_str: str):
        """Delegate to validator."""
        return self.validator.normalize_date(date_str)

    async def check_double_match(self, page: Page, reservation: dict):
        """Delegate to validator."""
        return await self.validator.check_double_match(page, reservation)

    async def fill_applicant_form(self, page: Page, person: dict, index: int = 0):
        """Delegate to form_filler."""
        return await self.form_filler.fill_applicant_form(page, person, index)

    async def fill_all_applicants(self, page: Page, reservation: dict):
        """Delegate to form_filler."""
        return await self.form_filler.fill_all_applicants(page, reservation)

    async def select_appointment_slot(self, page: Page, reservation: dict):
        """Delegate to slot_selector."""
        return await self.slot_selector.select_appointment_slot(page, reservation)

    async def select_preferred_time(self, page: Page):
        """Delegate to slot_selector."""
        return await self.slot_selector.select_preferred_time(page)

    async def handle_captcha_if_present(self, page: Page):
        """Delegate to slot_selector."""
        return await self.slot_selector.handle_captcha_if_present(page)

    async def fill_payment_form(self, page: Page, card_info: dict):
        """Delegate to payment_handler."""
        return await self.payment_handler.fill_payment_form(page, card_info)

    async def handle_3d_secure(self, page: Page, phone_number: str):
        """Delegate to payment_handler."""
        return await self.payment_handler.handle_3d_secure(page, phone_number)

    async def _wait_for_payment_confirmation(self, page: Page, max_wait: int = 60, check_interval: float = 2.0):
        """Delegate to payment_handler."""
        return await self.payment_handler._wait_for_payment_confirmation(page, max_wait, check_interval)

    async def verify_booking_confirmation(self, page: Page):
        """Delegate to validator."""
        return await self.validator.verify_booking_confirmation(page)
