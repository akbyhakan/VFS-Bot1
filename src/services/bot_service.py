...
class VFSBot:
    ...
    def __init__(self, config: Dict[str, Any], db: Database, notifier: NotificationService):
        ...
        # Thread safety for circuit breaker state
    ...
    async def _record_error(self) -> None:
        """Record error for circuit breaker tracking with thread safety."""
    ...
    async def _reset_circuit_breaker(self) -> None:
        """Reset circuit breaker state with thread safety."""
        async with self._circuit_breaker_lock:
            self.circuit_breaker_open = False
    ...
    async def fill_personal_details(self, page: Page, details: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            await wait_for_selector_smart(
                page, "input#first_name", timeout=Timeouts.SELECTOR_WAIT
            )
            ...
            await smart_fill(
                page, "input#last_name", details.get("last_name", ""), self.human_sim
            )
            await smart_fill(
                page,
            ...
            await smart_fill(
                page, "input#email", details.get("email", ""), self.human_sim
            )
    ...
    async def book_appointment(self, page: Page, date: str, time: str) -> Optional[str]:
        ...
        try:
            ...
            await wait_for_selector_smart(
                page, ".confirmation", timeout=Timeouts.SELECTOR_WAIT
            )
