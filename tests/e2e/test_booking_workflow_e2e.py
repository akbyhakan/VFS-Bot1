...
class TestBookingWorkflowE2E:
    ...
    async def test_full_process_user_flow(
        ...
        # Mock dependencies
        with patch('src.services.bot.auth_service.AuthService') as MockAuthService, \
            ...
            mock_slot_checker.check_slots = AsyncMock(return_value={
                "available": True,
                "slots": [{"date": "2024-12-25", "time": "10:00"}],
            })
            
            mock_booking = MockBooking.return_value
            mock_booking.book_appointment = AsyncMock(return_value={
                "success": True,
    ...
    @pytest.mark.asyncio
    async def test_login_failure_stops_flow(
        self,
        test_db: Database,
        user_repo: UserRepository
    ):
