...
class TestMetricsHelper:
    ...
    def test_record_booking_failure(self):
        ...
        # Should not raise
        MetricsHelper.record_booking_failure(centre="Istanbul", reason="payment_failed")
    ...
    def test_set_circuit_breaker_state(self):
        ...
        MetricsHelper.set_circuit_breaker_state(service="vfs_api", is_open=True)

        # Should not raise - closed
        MetricsHelper.set_circuit_breaker_state(service="vfs_api", is_open=False)

    def test_record_http_request(self):
        """Test recording HTTP request."""
        # Should not raise
        MetricsHelper.record_http_request(method="POST", endpoint="/api/appointments", status=200)

    def test_record_db_query(self):
        """Test recording database query."""
        # Should not raise - success
        MetricsHelper.record_db_query(operation="select", duration=0.015, success=True)

        # Should not raise - failure
        MetricsHelper.record_db_query(operation="insert", duration=0.025, success=False)

    def test_record_captcha_solved(self):
        """Test recording captcha solving."""
        # Should not raise
        MetricsHelper.record_captcha_solved(solver="2captcha", duration=15.5, success=True)

    def test_record_error(self):
        """Test recording error."""
        # Should not raise
        MetricsHelper.record_error(error_type="timeout", component="payment_service")
