...
class TestBezierCurve:
    ...
    def test_bezier_curve_points(self):
        ...
        assert points[0] == start or (abs(points[0][0] - start[0]) < 0.1 and abs(points[0][1] - start[1]) < 0.1)
        assert points[-1] == end or (abs(points[-1][0] - end[0]) < 0.1 and abs(points[-1][1] - end[1]) < 0.1)
