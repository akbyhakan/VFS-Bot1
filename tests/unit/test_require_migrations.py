...
async def test_missing_alembic_raises_error_when_required():
    ...
    with patch.dict(os.environ, {"REQUIRE_MIGRATIONS": "true"}):
        ...
        mock_acquire_cm.__aexit__ = AsyncMock(return_value=None)  # Return None to not suppress exceptions
        ...
        with patch("src.models.db_connection.asyncpg.create_pool", new=AsyncMock(return_value=mock_pool)):
...
async def test_reconnect_skips_migration_check():
    ...
    with patch.dict(os.environ, {"REQUIRE_MIGRATIONS": "true"}):
        ...
        # Mock asyncpg.create_pool in the correct module
        with patch("src.models.db_connection.asyncpg.create_pool", new=AsyncMock(return_value=mock_pool)) as mock_create:
